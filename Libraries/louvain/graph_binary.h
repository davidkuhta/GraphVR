...
class GraphB {
 public:
  ...
  GraphB(char *filename, char *filename_w, int type);

  GraphB(stringstream& data_stream, int type);
...
inline long double
GraphB::weighted_degree(int node) {
  assert(node>=0 && node<nb_nodes);
